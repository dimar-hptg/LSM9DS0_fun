...
void lily_fun()
{
	...
	const int low_fall_limit = 2500; // (0.5g*100)^2

	my_printf("[I2C] - Start %d\r\n", TEST_ID);

	// Init I2C and GPIOs
	initI2C();

	// Check if gyro and accelerometer are alive
	my_printf("Who am I: 0x%x\r\n", gReadByte(WHO_AM_I_G));
	my_printf("Who am I: 0x%x\r\n", xmReadByte(WHO_AM_I_XM));
	...
	for(i = 0; i < 10000; i++)
	{
		...
		if( (i % 100) == 0)
		{
			my_printf("Gyro X, Y, Z (dps * 100) %d, %d, %d\r\n", gx, gy, gz);
			my_printf("Accel X, Y, Z (g * 100): %d, %d, %d\r\n", ax, ay, az);
			my_printf("Mag X, Y, Z (Gs * 100) %d, %d, %d\r\n", mx, my, mz);
		}

		// Check the value of acceleration vector
		if((ax*ax + ay*ay + az*az) < low_fall_limit)
		{
			my_printf("FALL\r\n");
		}
